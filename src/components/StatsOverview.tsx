...
export function StatsOverview() {
  ...
  type RankingGroupFilter = "ALL" | "MY_GROUP";
  const [rankingGroupFilter, setRankingGroupFilter] =
  useState<RankingGroupFilter>("ALL");
    ...
    useEffect(() => { 
      ...
      .then((data) => { 
        ...
        useEffect(() => {
          ...
          getUserRanking(rankingPeriod, 5,  rankingGroupFilter === "MY_GROUP" ? myAdminId : undefined)
            .then((data) => {
              setTopUser(data?.topUser ?? null);
              setRankingList(data?.rankingList ?? []);
            })
            .finally(() => setLoadingRanking(false));
        }, [rankingPeriod, rankingGroupFilter]);
...
<Card className="mt-6 joyride-pot-toprank">
<CardHeader className="pb-3">
  <div className="flex items-center justify-between">
    ...
    <div className="flex gap-3">
      {/* 🔹 그룹 필터 박스 */}
      <div className="flex gap-1 p-1 rounded-md border bg-muted/40 joyride-pot-group">
        <button
          onClick={() => setRankingGroupFilter("ALL")}
          className={`px-2 py-1 text-xs rounded ${
        ...
        <button
          onClick={() => setRankingGroupFilter("MY_GROUP")}
          className={`px-2 py-1 text-xs rounded ${
            rankingGroupFilter === "MY_GROUP"
              ? "bg-primary text-white"
              : "text-muted-foreground"
          }`}
        >
      ...
      {/* 🔹 기간 필터 박스 */}
      <div className="flex gap-1 p-1 rounded-md border bg-muted/40 joyride-pot-rank-filter">
        <button
  ...
  <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    {topUser && (
      <Card>
        ...
        <CardContent className="space-y-2 text-sm">
          ...
          <div className="flex justify-between">
            <span className="text-muted-foreground">화분 레벨</span>
            <span>Lv.{topUser.potLevel}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">쿠폰 사용</span>
            <span>
              {rankingPeriod === "weekly"
                ? topUser.weeklyCouponsUsed
                : topUser.totalCouponsUsed}
              회
            </span>
          </div>
        </CardContent>
    ...
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm">TOP 5 사용자</CardTitle>
      </CardHeader>
